...
#barplot(faces.ppc$sdev[1:20]^2)
#sppc <- summary(faces.ppc)
#importance <- sppc$importance[3,]
#proportion.of.variance <- sppc$importance[2,]
#barplot(importance[1:20],las=2,cex.names=0.7,main="Components vs the proportion of the variance explained")
...
#Center the matrix
f.photo.matrix.centered <- scale(f.photo.matrix,center = TRUE, scale = FALSE)
...
f.face.reconstructed <- (as.vector(f.matrix)-as.vector(f.mean.face)) %*% f.eigen.faces %*% t(f.eigen.faces)
...
#apply to apply old eigen vectors to the photo yaleB05_P00A+010E+00.pgm
f.face.reconstructed <- (as.vector(f.matrix)-as.vector(f.mean.face)) %*% eigen.faces %*% t(eigen.faces)
dim(f.face.reconstructed) <- photo.dim
f.face.reconstructed <- f.face.reconstructed + f.mean.face
f.face = pixmapGrey(f.face.reconstructed)
plot(f.face)
